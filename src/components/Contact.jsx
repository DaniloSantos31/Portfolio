            ...
            <div className="max-w-4xl mx-auto text-center">
                <div className={`transition-all duration-1000 delay-500
                    ...
                    'opacity-0 translate-y-10'}`}>
                        ...
                        gap-4 sm:gap-8">
                            <a href={contactLinksWithIcons[0].href}
                            className={`flex items-center justify-center w-full
                            sm:w-auto ${contactLinksWithIcons[0].className}`}>
                                {contactLinksWithIcons[0].icon}
                                {contactLinksWithIcons[0].label}
                            </a>
                            <div className="flex gap-4 sm:gap-8">
                                <a href={contactLinksWithIcons[1].href}
                                className={`flex items-center justify-center flex-1 sm:flex-none
                                ${contactLinksWithIcons[1].className}`}>
                                {contactLinksWithIcons[1].icon}
                                {contactLinksWithIcons[1].label}
                                </a>
                                <a href={contactLinksWithIcons[2].href}
                                className={`flex items-center justify-center flex-1 sm:flex-none
                                ${contactLinksWithIcons[2].className}`}>
